...
// this is fucked for async reasons
fs.readdir(assetDir, function(err, items) {
    // console.log(items);
    ...
    for (var i=0; i<items.length; i++) {
		...
		const json = {
			resourceDir: assetDir + "/" + items[i],
			binDir: "./bin/material/" + items[i],
			fontName: "Material Icons",
			fontId: "Material-Icons-" + items[i].capitalize(),
			fontFileName: "Material-Icons-" + items[i].capitalize(),
			fontStyle: items[i].capitalize(),
			fontBoxSize: 24,
			icons: []
		};
