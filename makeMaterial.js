...
const cliDefinitions = [
	...
	},
	{
		...
		type: String,
		default: true,
		description: 'style of Material Font to generate. Value must be one of the following (Baseline | Outline | Round | Sharp). Baseline will be used if none is given.'
	}
]

let options = commandLineArgs(cliDefinitions);

if (options.help) {
	const sections = [
		{
			header: 'zms2hrone',
			content: 'generates Material Design Font files in the given style.\nUsage:  node makeMaterial.js Outline'
		},
		{
			header: 'Options',
			optionList: cliDefinitions
		}
	];
	...
	if (!styleName.match(/(Baseline)|(Outline)|(Sharp)|(Round)/gi)) {
		throw 'invalid style name given ' + styleName + '\nMust be one of Baseline | Outline | Round | Sharp';
	};
	...
	exports.scrape = function (sourceURL) {
		...
		fs.readdir(sourceURL, function (err, list) {
			...
			// sort in order to always generate consistent font
	...
	function writeTTF() {
		...
		console.log('ttf successfully created! - style ' + styleName);
	}
	...
	function writeWOFF() {
		...
		console.log('woff successfully created! - style ' + styleName)
	}
	...
	function writeWOFF2() {
		...
		console.log('woff2 successfully created! - style ' + styleName)
	}
	...
	function writeEOT() {
		...
		console.log('eot successfully created! - style ' + styleName)
	}
